...
let gameState = {
    board: [], // Store the board configuration
    activePlayer: 'W', // 'W' for white, 'B' for black
    history: [] // Store the game state history for undo functionality
};

// Function to initialize the game state
function initializeGameState() {
    gameState.board = [...document.querySelectorAll('.box')].map(box => box.innerText);
}

// Function to update the game state
function updateGameState() {
    gameState.board = [...document.querySelectorAll('.box')].map(box => box.innerText);
    gameState.activePlayer = tog % 2 !== 0 ? 'W' : 'B';
}

// Function to revert to the previous game state (Undo)
function undo() {
...
// Function to render the game based on the provided state
function render(state) {
    state.board.forEach((value, index) => {
        document.getElementById(`b${index}`).innerText = value;
    });
    tog = state.activePlayer === 'W' ? 1 : 2;
    document.getElementById('tog').innerText = state.activePlayer === 'W' ? "White's Turn" : "Black's Turn";
}

// Function to save current game state to history
...
// Modify your existing event listener to save game state before making changes
document.querySelectorAll('.box').forEach(item => {
    item.addEventListener('click', function () {
        saveToHistory();
...
// Call initializeGameState() to initialize the game state at the beginning
initializeGameState();
